...
import { ChevronRight, BookOpenText, PenLine } from "lucide-react";

import { StoryCard } from "@/components/safarnama/story-card";
import { Reveal } from "@/components/ui/reveal";
import { type StoryRow } from "@/lib/safarnama";

/** Homepage Safarnama strip — real approved traveller stories (CTA fallback). */
export function SafarnamaSection({ items }: { items: StoryRow[] }) {
  return (
      ...
      <div className="container-px">
        <Reveal className="mb-6 flex flex-col items-start gap-1.5 sm:flex-row sm:items-end sm:justify-between sm:gap-3">
          <div className="min-w-0">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-forest-100 px-3 py-1 text-xs font-bold uppercase tracking-wide text-forest-700">
              <BookOpenText className="h-3.5 w-3.5" /> Community
            </span>
            <h2 className="mt-2 font-display text-lg font-bold uppercase leading-tight text-forest sm:text-2xl lg:text-3xl">
              Safarnama — Traveler Stories
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Real journeys, budgets and tips shared by travellers who&apos;ve explored Gilgit-Baltistan.
            </p>
          </div>
          <Link
            href="/safarnama"
        ...
        {items.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-between gap-4 rounded-3xl border border-border/70 bg-gradient-to-br from-forest-50/60 to-gold/5 p-8 text-center shadow-premium sm:flex-row sm:text-left">
            <div>
              <h3 className="font-display text-lg font-bold text-forest">Been to Gilgit-Baltistan?</h3>
              <p className="mt-1 text-sm text-muted-foreground">
                Share your journey and help fellow travellers plan a better trip.
              </p>
            </div>
            <Link
