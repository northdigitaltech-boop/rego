...
import {
  ...
  deleteHomestay,
  getRoomsByHomestay,
  replaceHomestayRooms,
  getBookingRoomsForHomestays,
  type HomestayRow,
  type HomestayRoomInput,
  type HomestayBookingRoomRow,
} from "@/lib/homestays";
import {
  getHomestayBookingsByOwner,
  setHomestayBookingStatus,
  homestayBookingRef,
  type HomestayBookingRow,
} from "@/lib/homestay-bookings";
import { setHomestayBookingRoomsStatus } from "@/lib/homestays";
...
const nav: { id: Tab; label: string; icon: typeof LayoutDashboard }[] = [
  ...
  { id: "homestays", label: "My Homestays", icon: Home },
  ...
  { id: "add", label: "Add Homestay", icon: PlusCircle },
...
export function HomestayProviderDashboard({
  user,
  onSignOut,
}: {
  user: User;
  ...
  const [homestays, setHomestays] = React.useState<HomestayRow[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [editingId, setEditingId] = React.useState<string | null>(null);
  const [form, setForm] = React.useState({ ...empty });
  const [saving, setSaving] = React.useState(false);
  const [error, setError] = React.useState("");
  const [bookings, setBookings] = React.useState<HomestayBookingRow[]>([]);
  const [brooms, setBrooms] = React.useState<HomestayBookingRoomRow[]>([]);
  const [chatBooking, setChatBooking] = React.useState<BookingRow | null>(null);

  const refresh = React.useCallback(async (silent = false) => {
    if (!silent) setLoading(true);
    const [h, b] = await Promise.all([
      getHomestaysByOwner(user.email),
      getHomestayBookingsByOwner(user.email),
    ]);
    setHomestays(h);
    setBookings(b);
    setBrooms(await getBookingRoomsForHomestays(h.map((x) => x.id)));
  ...
  // The shared chat UI expects the hotel BookingRow shape; adapt homestay rows.
  const toBookingRow = (b: HomestayBookingRow): BookingRow => ({
    ...b,
  ...
  const openChat = (b: HomestayBookingRow) => openChatRow(toBookingRow(b));
  ...
  const totalRooms = homestays.reduce((s, x) => s + (x.total_rooms || 0), 0);
  ...
  const handleBookingAct = async (
    ...
    const b = bookings.find((x) => x.id === id);
    await setHomestayBookingStatus(id, status);
    await setHomestayBookingRoomsStatus(id, status);
    if (b) {
      sendEmail(
        b.customer_email,
        status === "accepted"
          ? "Your Rego homestay booking is confirmed"
          : "Rego booking update",
        bookingStatusEmail({
          name: b.customer_name ?? "Guest",
          hotel: b.homestay_title,
          ref: homestayBookingRef(b.id),
      ...
      await sendBookingStatusNotification({
        ...
        owner_name: b.homestay_title ?? user.name,
        ref: homestayBookingRef(b.id),
        itemTitle: b.homestay_title,
        status,
      });
    }
    await refresh();
  ...
  const startEdit = async (h: HomestayRow) => {
    setEditingId(h.id);
    const rooms = await getRoomsByHomestay(h.id);
    setForm({
      title: h.title,
  ...
  const save = async (e: React.FormEvent) => {
    ...
    setError("");
    if (!form.title.trim()) {
      setError("Please enter a homestay name.");
      return;
    }
    const roomInputs: Omit<HomestayRoomInput, "homestay_id">[] = form.rooms
    ...
    const payload = {
      ...
      category_label: form.category_label.trim() || "Homestay",
      ...
      image:
        form.image.trim() ||
        `https://picsum.photos/seed/homestay-${
    ...
    const homestayId = editingId ?? (data as HomestayRow | null)?.id;
    if (homestayId) {
      await replaceHomestayRooms(homestayId, roomInputs);
    }
    setSaving(false);
    resetForm();
    await refresh();
  ...
  return (
    <div className="container-px py-10">
      ...
      <p className="mt-1 text-muted-foreground">
        Manage your homestay listings — they go live once an admin approves them.
      </p>
      <div className="mt-3">
              ...
              {nav.map((n) => {
                ...
                return (
                  ...
                  >
                    ...
                    {n.id === "add" && editingId ? "Edit Homestay" : n.label}
        ...
        <div>
          ...
          {tab === "payments" && (
            <div className="space-y-8">
              <PaymentsManager table="homestay_bookings" bookings={bookings} onChange={refresh} senderEmail={user.email} senderName={user.name} />
              <PaymentSettings tables={["homestays"]} ownerEmail={user.email} />
            </div>
          )}
          ...
          {tab === "restaurant" && (
            <PropertyRestaurantManager
              ...
              propertyId={homestays[0]?.id ?? null}
              propertyName={homestays[0]?.title ?? null}
            />
          )}
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-3xl border border-border/70 bg-card p-8 text-center shadow-premium">
        ...
        <h2 className="mt-3 font-display text-lg font-bold text-forest">
          {count === 0 ? "Add your first homestay" : "Add another homestay"}
        </h2>
        <p className="mt-1 text-muted-foreground">
          Submit your homestay — it appears on the site once an admin approves it.
        </p>
...
function MyHomestays({
  ...
  homestays: HomestayRow[];
  loading: boolean;
  onEdit: (h: HomestayRow) => void;
    ...
    return (
      <div className="rounded-3xl border border-dashed border-border bg-muted/40 py-16 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Add your homestay to publish it on the site.
        </p>
        <Button variant="gold" className="mt-5 rounded-lg" onClick={onAdd}>
          Add homestay
        </Button>
    ...
    >
      <div>
        <h2 className="font-display text-xl font-bold text-forest">
          {editingId ? "Edit homestay" : "Add a new homestay"}
        </h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Add your homestay details, room types and availability — all in one place.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Homestay name" required>
          <input
            value={form.title}
            onChange={(e) => setForm({ ...form, title: e.target.value })}
            placeholder="e.g. Hunza View Homestay"
        ...
        <Field label="Type / label">
          <input
            ...
            placeholder="Homestay / Guest House / Farm Stay"
            className="auth-input"
          />
        </Field>
      </div>
      ...
      <Field label="Description">
        <textarea
          ...
          placeholder="Describe your homestay, the area and what makes it special…"
...
function BookingsPanel({
  ...
  bookings: HomestayBookingRow[];
  loading: boolean;
  onAct: (id: string, status: "accepted" | "rejected") => void;
  onChat: (b: HomestayBookingRow) => void;
  unread: Set<string>;
}) {
  if (loading) {
    return (
    ...
    return (
      <div className="rounded-3xl border border-dashed border-border bg-muted/40 py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          When guests book your homestays, requests appear here.
        </p>
      </div>
    );
  }
